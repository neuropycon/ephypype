...
def format_electrodes_xls(data_path,xls_electrodes_file):
    ...
    df.loc[pd.notnull(df["replace or remove"]),"name_brain region"] = df["replace or remove"][pd.notnull(df["replace or remove"])]
    ...
    for animal_id in np.unique(df['animal']):
        ...
        rat_electrode_file = os.path.join(data_path,animal_id + "_electrode_modif.txt")
