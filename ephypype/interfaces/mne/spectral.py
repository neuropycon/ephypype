...
class SpectralConnInputSpec(BaseInterfaceInputSpec):
    ...
    con_method = traits.Enum("coh", "imcoh", "plv", "pli", "wpli", "pli2_unbiased", "ppc",
                             "cohy", "wpli2_debiased", desc='metric computed on time series for connectivity')

    epoch_window_length = traits.Float(-1.0,
                                       desc='epoched data', mandatory=False)

    export_to_matlab = traits.Bool(
        False, desc='If conmat is exported to .mat format as well', usedefault=True)

    index = traits.Int(
        0, desc="What to add to the name of the file", usedefault=True)

    multi_con = traits.Bool(
        False, desc='If multiple connectivity matrices are exported', usedefault=True)
...
class SpectralConnOutputSpec(TraitedSpec):
    ...
    conmat_files = traits.List(
        File(exists=False), desc="all spectral connectivty matrices in .npy format")
...
class SpectralConn(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if epoch_window_length == traits.Undefined:
            ...
            print(("epoching data with {}s by window, resulting in {} epochs (rest = {})".format(
        ...
        if multi_con:
            self.conmat_files = compute_and_save_multi_spectral_connectivity(
                all_data=data, con_method=con_method, sfreq=sfreq, fmin=freq_band[0], fmax=freq_band[1], export_to_matlab=export_to_matlab, mode=mode)

        else:
            self.conmat_file = compute_and_save_spectral_connectivity(
                data=data, con_method=con_method, index=index, sfreq=sfreq, fmin=freq_band[0], fmax=freq_band[1], export_to_matlab=export_to_matlab, mode=mode)
...
class PlotSpectralConn(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        assert conmat.ndim == 2, "Warning, conmat should be 2D matrix , ndim = {}".format(
            conmat.ndim)
        assert conmat.shape[0] == conmat.shape[1], "Warning, conmat should be a squared matrix , {} != {}".format(
...
#                rh_labels = [label for (yp, label) in sorted(zip(label_ypos_rh, rh_labels))]
        ...
        self.plot_conmat_file = plot_circular_connectivity(
            conmat, label_names, node_colors, node_order, vmin, vmax, nb_lines, fname)
