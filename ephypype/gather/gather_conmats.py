...
def plot_tab_circular_connectivity(list_list_conmat,all_elec_labels,plot_filename,coh_low_thresh = 0.0, coh_high_thresh = 1.0, color_bar = 'gist_rainbow', column_labels = [], row_labels = []):
    ...
    fig,axes = plt.subplots(nrows=nb_lines, ncols=nb_cols, figsize=(4*nb_cols, 4*nb_lines), facecolor='black')
    ...
    assert len(column_labels) == nb_cols,"Error, specifying invalid number of column labels"
    assert len(row_labels) == nb_lines,"Error, specifying invalid number of line labels"
    ...
    for index_sess,list_conmat in enumerate(list_list_conmat):
        ...
        for index_win, np_all_mean_con_mats in enumerate(list_conmat):
            ...
            if index_win == len(list_conmat)-1:
                ...
                fig, ax = plot_connectivity_circle(np_all_mean_con_mats, all_elec_labels, textcolor = "black", facecolor = "white",n_lines=None,  node_angles= all_node_angles, fontsize_names = 15, colorbar= False, show = False,colormap = color_bar, vmin = coh_low_thresh, vmax = coh_high_thresh,fig = fig, subplot = (nb_lines,nb_cols,1+index_win+nb_cols*index_sess))
            
            if index_win == 0:
                ax.set_ylabel(row_labels[index_sess], rotation=0, size='large',fontsize = 25)
